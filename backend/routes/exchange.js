...
// Get open orders
router.get('/:userId/:platform/orders/open', async (req, res, next) => {
  try {
    const { userId, platform } = req.params;
    const { symbol } = req.query;

    console.log(`[EXCHANGE OPEN ORDERS] Getting open orders for user: ${userId}`);
    ...
    if (platform === 'binance') {
      ...
      try {
        const response = await axios.get(
          `https://api.binance.com/api/v3/openOrders?${queryString}&signature=${signature}`,
          {
            headers: {
              'X-MBX-APIKEY': apiKey,
            },
        ...
        console.log(`[EXCHANGE OPEN ORDERS] ✅ Got ${response.data.length} open orders`);
        res.json({
          success: true,
        ...
        console.error(`[EXCHANGE OPEN ORDERS] ❌ Binance API error:`, apiError.response?.data || apiError.message);
        return res.status(400).json({
          success: false,
      ...
      res.status(400).json({
        success: false,
        error: `Open orders for ${platform} not implemented yet`,
      });
    }
  } catch (error) {
    console.error(`[EXCHANGE OPEN ORDERS] ❌ Error:`, error);
    next(error);
  }
});

// Cancel order
router.delete('/:userId/:platform/order', async (req, res, next) => {
  try {
    const { userId, platform } = req.params;
    const { symbol, orderId } = req.body;

    console.log(`[EXCHANGE CANCEL ORDER] Canceling order ${orderId} for user: ${userId}`);
    ...
    if (platform === 'binance') {
      ...
      try {
        ...
        console.log(`[EXCHANGE CANCEL ORDER] ✅ Order cancelled successfully`);
        res.json({
          success: true,
        ...
        console.error(`[EXCHANGE CANCEL ORDER] ❌ Binance API error:`, apiError.response?.data || apiError.message);
        return res.status(400).json({
          success: false,
      ...
      res.status(400).json({
        success: false,
        error: `Order cancellation for ${platform} not implemented yet`,
      });
    }
  } catch (error) {
    console.error(`[EXCHANGE CANCEL ORDER] ❌ Error:`, error);
    next(error);
  }
});

// Get order history
router.get('/:userId/:platform/orders/history', async (req, res, next) => {
  try {
    const { userId, platform } = req.params;
    const { symbol, limit } = req.query;

    console.log(`[EXCHANGE ORDER HISTORY] Getting order history for user: ${userId}`);
    ...
    if (platform === 'binance') {
      ...
      const queryString = `symbol=${symbol}&limit=${limit || 50}&timestamp=${timestamp}`;
      ...
      try {
        ...
        console.log(`[EXCHANGE ORDER HISTORY] ✅ Got ${response.data.length} orders`);
        res.json({
          success: true,
          data: response.data.map(order => ({
            orderId: order.orderId,
        ...
        console.error(`[EXCHANGE ORDER HISTORY] ❌ Binance API error:`, apiError.response?.data || apiError.message);
        return res.status(400).json({
          success: false,
      ...
      res.status(400).json({
        success: false,
        error: `Order history for ${platform} not implemented yet`,
      });
    }
  } catch (error) {
    console.error(`[EXCHANGE ORDER HISTORY] ❌ Error:`, error);
    next(error);
  }
});

// Test order (doesn't actually place order - useful for testing)
router.post('/:userId/:platform/order/test', async (req, res, next) => {
  try {
    const { userId, platform } = req.params;
    const { symbol, side, type, quantity, price } = req.body;

    console.log(`[EXCHANGE TEST ORDER] Testing order for user: ${userId}`);
    ...
    if (platform === 'binance') {
      ...
      try {
        // Use the test endpoint - this validates but doesn't execute
        const response = await axios.post(
        ...
        console.log(`[EXCHANGE TEST ORDER] ✅ Test order validated successfully`);
        res.json({
          success: true,
          message: 'Order validation successful - order would be accepted',
          data: {
            symbol,
        ...
        console.error(`[EXCHANGE TEST ORDER] ❌ Binance API error:`, apiError.response?.data || apiError.message);
        return res.status(400).json({
          success: false,
      ...
      res.status(400).json({
        success: false,
        error: `Test order for ${platform} not implemented yet`,
      });
    }
  } catch (error) {
    console.error(`[EXCHANGE TEST ORDER] ❌ Error:`, error);
    next(error);
  }
});
